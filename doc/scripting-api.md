...
| `button(message: String): Void` | Requests button click from the user via web socket and returns when the user clicks on the button. Returns immediately if the script is not running interactively. | `Input.button("Continue");` |
